...
def convert_markdown_to_utf8(input_file: str, output_file: str = None):
    ...
    print(f"[Encoding] Encoding detected : {encoding}")
    if encoding.lower().replace("-", "") == 'utf8':
        print("[Encoding] File is already encoded as utf-8")
        if output_file is not None:
            print(f"[Encoding] Copying {input_file} to {output_file}")
    ...
    print(f"[Encoding] File '{input_file}' converted to UTF-8 and saved as '{output_file}'")
